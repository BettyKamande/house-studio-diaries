...
const PortfolioHero = () => {
    ...
    return (
        <section className="w-full h-96 overflow-hidden sm:h-screen">
            {portfolios.map((portfolio, index) => (
                <div key={portfolio.id} className={`w-full h-full bg-cover bg-center relative ${currentIndex === index ? "block" : "hidden"}`} style={{ backgroundImage: `url(${portfolio.img})`}}>
                    ...
                    <div className="absolute w-full h-max text-white flex justify-between items-end z-10 bottom-5 px-5 sm:bottom-10 sm:px-10  lg:bottom-20 lg:px-20 ">
